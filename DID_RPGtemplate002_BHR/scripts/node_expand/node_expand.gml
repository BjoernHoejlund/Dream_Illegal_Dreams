/// @param node
///node_expand(node)
function node_expand(argument0) {
	...
	if (node.object_index == obj_root_node) {
	  ...
	  for (var i = 0; i < array_length(node.preActions); i++) {
	  ...
	  for (var i = 0; i < array_length(node.actions); i++) {
	    action_deactivate(node.actions[i]);
	  }
  
	  node.final_text = node.child_text;
	  for (var i = 0; i < array_length(node.mods); i++) {
